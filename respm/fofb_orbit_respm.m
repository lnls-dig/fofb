function [Mcorr, Mss, Mdisp, Mrf] = fofb_orbit_respm(THERING, markers, plane)
...
if isfield(markers, 'corr') && ~isempty(markers.corr)
    ...
    Mcorr_ = corrector_respm(THERING, markers.orbit, markers.corr, cellstr(repmat(plane,length(markers.corr),1)));
...
if isfield(markers, 'rf') && ~isempty(markers.rf)
    ...
    Mrf_ = rf_respm(THERING, markers.orbit, markers.rf);
...
if isfield(markers, 'disp') && ~isempty(markers.disp)
    ...
    Mdisp_ = displacement_respm(THERING, markers.orbit, markers.disp, cellstr(repmat(plane,length(markers.disp),1)));
...
if isfield(markers, 'ss') && ~isempty(markers.ss)
    ...
    Mss_ = corrector_respm(THERING, markers.orbit, markers.ss, cellstr(repmat(plane,length(markers.corr),1)));
