...
% Parameters
amplitude = 0.03; % RMS value (randn) or sinusoid amplitude
expduration = 100; % each experiment's duration (packet count)
profile_mode = 'corr';
excitation_mode = 'randn';
freqs = [1:10 20:100 200:500];
Ts = 320e-6;
% ---
...
if ischar(varargin{1}) && strcmpi(varargin, 'size')
    ...
    if rem(i,(expduration+1)) == 0
        ...
        elseif strcmpi(excitation_mode, 'sin_sweep')
            ...
            packet = amplitude*repmat(sin(2*pi*f*t), 1, size(profile, 1))*profile;
        end
...
if ischar(argin1) && strcmpi(argin1, 'size')
    if strcmpi(mode, 'corr')
        argout1 = 43;
    elseif  strcmpi(mode, 'corr_profiles')
        argout1 = 42;
    elseif  strcmpi(mode, 'orbit_profiles')
        argout1 = 50;
    end
else
    i = argin1;
    ...
    elseif  strcmpi(mode, 'corr_profiles') || strcmpi(mode, 'orbit_profiles')
