...
manual_settings_array = {...
    ...
    {'AWG01 = closed, AON11 = closed', 1370}, ...
    {'SCW = 1.4 T', 1370}, ...
    {'SCW = 3.5 T, user condition (correct tune, coupling, etc.)', 1370}, ...
...
for j=1:length(exp_descriptions)
    if strcmpi(exp_descriptions{j}{1}, 'manual_settings')
        ...
        if strcmpi(expinfo.excitation, 'sine')
            expinfo.duration = floor(expinfo.duration/expinfo.nperiods);
